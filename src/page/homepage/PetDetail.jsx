...
export default function PetDetail() {
  ...
  const fetchPetData = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`http://localhost:5000/api/pets/get-listing/${id}`);
...
const handleMessageClick = async () => {
  ...
  if (user._id === petData.owner) { // This check should compare IDs.
  ...
  try {
    const payload = {
      petId: petData._id,
      requesterId: user._id, // The logged-in user is the requester (string ID)
    ...
    const response = await axios.post('http://localhost:5000/api/chat/create-channel', payload, {
      headers: {
    ...
    if (response.data.success && response.data.channelId) {
      console.log("Channel created/retrieved successfully:", response.data.channelId);
      navigate(`/chat/${response.data.channelId}`);
    } else {
      console.error("Failed to create chat channel (backend response):", response.data.error || "Unknown error");
      alert(`Failed to create chat channel: ${response.data.error || "Unknown error"}`);
    }
  } catch (error) {
    console.error("Error creating chat channel (Axios error):", error);
    // Access more specific error details from Axios
    const errorMessage = error.response?.data?.error || error.message || 'Failed to create chat channel';
      ...
      <>
        ...
        <div className="min-h-screen bg-gray-50 flex items-center justify-center">
          <div className="text-center max-w-md p-6 bg-white rounded-xl shadow-lg">
            ...
            <h2 className="text-xl font-bold text-gray-900 mb-3">Error Loading Pet</h2>
  ...
  const publishDate = petData.createdAt ? formatDate(petData.createdAt) : "Unknown date";
              ...
              <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
                ...
                <div className="flex gap-3">
                  <button 
                    ...
                    disabled={user && user._id === petData.owner._id}
                    className={`flex-1 bg-[#A0C878] text-white px-6 py-4 rounded-xl font-semibold hover:shadow-lg transform hover:scale-[1.02] transition-all duration-300 ${
                      user && user._id === petData.owner._id ? 'opacity-50 cursor-not-allowed' : ''
                    }`}
                  >
                    Adopt {petData.name}
                  </button>
                  <button 
                    ...
                    disabled={isCreatingChannel || (user && user._id === petData.owner._id)}
                    className={`px-6 py-4 border-2 border-[#A0C878] text-gray-700 rounded-xl font-semibold hover:border-gray-400 hover:bg-gray-50 transition-all duration-300 ${
                      isCreatingChannel || (user && user._id === petData.owner._id) ? 'opacity-50 cursor-not-allowed' : ''
                    }`}
                  >
