...
const Navbar = () => {
  ...
  const handlePremiumClick = () => {
    if (isAuthenticated) {
      ...
      // Arahkan ke signup jika belum login
      navigate("/signup");
    }
  };
  ...
  return (
    <>
      <header className="fixed top-0 w-full z-50 px-3 sm:px-4 py-2 sm:py-3">
        ...
        >
          ...
          <div className="flex-1">
            <Link
              to={"/"}
              className="max-w-min text-2xl font-bold text-gray-800"
            >
              <motion.img
                ...
                >
                  ...
                  {isActive && (
                    <motion.div
                      ...
                      className="absolute bottom-0 left-0 right-0 h-1 bg-[#A0C878] rounded-t-full transition-transform duration-200 hover:scale-105"
                      transition={{
                        type: "spring",
                        stiffness: 300,
                        damping: 25,
          ...
          <div className="hidden md:flex flex-1 justify-end items-center gap-2 sm:gap-4">
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="cursor-pointer bg-[#A0C878] text-white px-4 py-2 rounded-full text-sm font-bold shadow-lg flex items-center gap-2 hover:bg-[#8eb866] transition-colors w-max"
              onClick={handlePremiumClick} // Gunakan handler baru
            >
              <Crown size={16} />
              Get Premium
            </motion.button>
            ...
            {!isAuthenticated ? (
              <>
                <button
                  ...
                  className="cursor-pointer bg-white text-[#A0C878] border-2 border-[#A0C878] hover:bg-[#A0C878] hover:text-white px-3 sm:px-5 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-semibold transition-all duration-300 shadow-md hover:shadow-lg w-max"
                >
                  Sign In
                </button>
                <button
                      ...
                      <div className="py-1">
                        ...
                        <div className="py-1 border-t border-gray-100">
                          <button
                            ...
                            className="flex items-center px-3 sm:px-4 py-2 text-xs sm:text-sm text-red-600 hover:bg-gray-100 w-full text-left cursor-pointer"
                          >
          ...
          <div className="md:hidden flex items-center gap-2 sm:gap-4 ml-auto mr-2 md:mr-0">
            <motion.button
              animate={{ scale: [1, 1.05, 1] }}
              transition={{
                repeat: Infinity,
              ...
              whileHover={{ scale: 1.1 }}
              whileTap={{ scale: 0.95 }}
              className="cursor-pointer bg-[#A0C878] text-white px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-[10px] sm:text-sm font-bold shadow-lg flex items-center gap-1.5 sm:gap-2 hover:bg-[#8eb866] transition-colors w-max"
              onClick={handlePremiumClick}
            >
              <Crown size={14} className="sm:w-[16px]" />
                    ...
                    return (
                      ...
                      >
                        {isAuthenticated ? (
                          <Link
                            to={link.path}
                            className={`transition-transform duration-200 hover:scale-105 flex items-center py-3 px-6 text-sm ${
                              isActive
                          ...
                          >
                            {isActive && (
                              <motion.div
                                ...
                                className="w-1.5 h-1.5 rounded-full bg-[#A0C878] mr-2 transition-transform duration-200 hover:scale-105"
                              />
                            )}
                            {link.label}
                          </Link>
                          ...
                          <button
                            onClick={() => scrollTo(link.id)}
                            className={`flex items-center py-3 px-6 w-full text-left text-sm transition-transform duration-200 hover:scale-105 ${
                              isActive
      ...
      <PremiumModal
        ...
        featureSource="navbar_premium_button"
      />
    </>
  );
};
